...
class AppStarterAgent(ConversableAgent):
    profile: ProfileConfig = ProfileConfig(
        name=DynConfig(
            "AppStarter",
            category="agent",
            key="derisk_ant_agent_agents_app_resource_starter_assistant_agent_profile_name",
        ),
        role=DynConfig(
            "App Starter",
            category="agent",
            key="derisk_ant_agent_agents_app_resource_starter_assistant_agent_profile_role",
        ),
        goal=DynConfig(
            "根据用户的问题和提供的应用信息，从已知资源中选择一个合适的应用来解决和回答用户的问题,并提取用户输入的关键信息到应用意图的槽位中。",
            category="agent",
            key="derisk_ant_agent_agents_app_resource_starter_assistant_agent_profile_goal",
        ),
        constraints=DynConfig(
            ...
            category="agent",
            key="derisk_ant_agent_agents_app_resource_starter_assistant_agent_profile_constraints",
        ),
        desc=DynConfig(
            "根据用户问题匹配合适的应用来进行回答.",
            category="agent",
            key="derisk_ant_agent_agents_app_resource_starter_assistant_agent_profile_desc",
        ),
    )
