...
from derisk_serve.agent.agents.expand.actions.app_start_action import StartAppAction
...
class StartAppAssistantAgent(ConversableAgent):
    profile: ProfileConfig = ProfileConfig(
        name=DynConfig(
            "AppLauncher",
            category="agent",
            key="derisk_ant_agent_agents_app_start_assistant_agent_profile_name",
        ),
        role=DynConfig(
            "AppLauncher",
            category="agent",
            key="derisk_ant_agent_agents_app_start_assistant_agent_profile_role",
        ),
        goal=DynConfig(
            "The agent starts the selected application.",
            category="agent",
            key="derisk_ant_agent_agents_app_start_assistant_agent_profile_goal",
        ),
        constraints=DynConfig(
            [],
            category="agent",
            key="derisk_ant_agent_agents_app_start_assistant_agent_profile_constraints",
        ),
        desc=DynConfig(
            "The agent starts the selected application.",
            category="agent",
            key="derisk_ant_agent_agents_app_start_assistant_agent_profile_desc",
        ),
    )
    last_rounds: int = 0
