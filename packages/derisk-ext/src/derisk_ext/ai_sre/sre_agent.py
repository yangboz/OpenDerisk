...
from derisk.agent import ProfileConfig, Agent, AgentMessage, ActionOutput, ConversableAgent, GptsMemory
from derisk.agent.core.base_team import ManagerAgent
from derisk.agent.core.memory.gpts import GptsPlan, GptsPlansMemory
...
from derisk_ext.ai_sre.actions.planning_action import TaskPlan

logger = logging.getLogger(__name__)


class SreManager(ManagerAgent):
        ...
        try:
            ...
            for i in range(self.max_round):
                ...
                for task in task_params:
                    ...
                    await self.memory.gpts_memory.append_plans(conv_id=self.agent_context.conv_id, plans=[plan])
                ...
                if reporter_agent:
                    ...
                    api_tasks.append(
                        reporter_agent.generate_reply(
                            ...
                            historical_dialogues=sorted(all_task_messages, key=lambda obj: obj.rounds),  # 会变成多轮历史消息
                            force_use_historical=True
                        )
                    )
                ...
                for result in results:
                    ...
                    if result.name == reporter.name:
                        ...
                        return ActionOutput(is_exe_success=True, content="分析诊断已完成！")
                    else:
