...
from derisk.agent.util.llm.llm import _build_model_request
from derisk.core import LLMClient, BaseOutputParser, ModelRequestContext
...
class AIWrapper:
    ...
    async def create(self, verbose: bool = False, **config):
        ...
        async for out in self._completions_create(llm_model, params):
            yield out
        ...
        try:
            ...
            if stream_out:
                async for output in self._llm_client.generate_stream(
                    ...
                    model_output = output
                    content = self._output_parser.parse_model_nostream_resp(
                        model_output, "#@$"
                    )
                    if first_chunk:
                        if not content or len(content) <= 0:
                            continue
                    first_chunk = False
                    parsed_output = content.strip().replace("\\n", "\n")
                    end_time = datetime.now()
                    logger.info(
                        f"LLM stream generate cost:{end_time - start_time} "
                ...
                if verbose:
                    ...
                    logger.info(f"LLM Output[verbose]: \n{parsed_output}")
                    print("-" * 80, "\n", flush=True, sep="")
                yield parsed_output
