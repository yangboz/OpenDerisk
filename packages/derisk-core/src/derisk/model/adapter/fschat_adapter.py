...
from derisk.model.adapter.template import ConversationAdapter, PromptType
...
# If some model is not in the blacklist, but it still affects the loading of DERISK,
...
class FastChatLLMModelAdapterWrapper(LLMModelAdapter):
    ...
    def get_generate_stream_function(
        ...
        if _IS_BENCHMARK:
            from derisk.util.benchmarks.llm.fastchat_benchmarks_inference import (
                generate_stream,
            )
