...
from derisk.core.awel import BaseOperator, DAGVar, MapOperator
    ...
    metadata = ViewMetadata(
        ...
        parameters=[
            ...
            Parameter.build_from(
                ...
                options=VariablesDynamicOptions(),
                ui=ui.UIVariablesInput(
                    key="derisk.core.flow.params", scope=VARIABLES_SCOPE_FLOW_PRIVATE
                ),
            ),
            Parameter.build_from(
                "DAG Var 2",
...
def json_flow():
    ...
    name_to_parameters_dict = {
        "my_test_variables_operator": {
            "openai_api_key": "${derisk.model.openai.api_key:my_key@global}",
            "model": "${derisk.model.openai.model:default_model@global}",
            "dag_var1": "${derisk.core.flow.params:name1@%s}"
            % VARIABLES_SCOPE_FLOW_PRIVATE,
            "dag_var2": "${derisk.core.flow.params:name2@%s}"
            % VARIABLES_SCOPE_FLOW_PRIVATE,
        }
    }
        ...
        (
            {
                "vars": {
                    "openai_api_key": {
                        "key": "${derisk.model.openai.api_key:my_key@global}",
                        "value": "my_openai_api_key",
                        "value_type": "str",
                        "category": "secret",
                    },
                    "model": {
                        "key": "${derisk.model.openai.model:default_model@global}",
                        "value": "GPT-4o",
                        "value_type": "str",
                    },
                }
