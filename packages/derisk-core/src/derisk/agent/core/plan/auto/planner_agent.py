...
class PlannerAgent(ConversableAgent):
    ...
    profile: ProfileConfig = ProfileConfig(
        name=DynConfig(
            "Planner",
            category="agent",
            key="derisk_agent_plan_planner_agent_profile_name",
        ),
        role=DynConfig(
            "Planner",
            category="agent",
            key="derisk_agent_plan_planner_agent_profile_role",
        ),
        goal=DynConfig(
            ...
            "user's problem, generating a task reasoning_engine that can be completed through"
            " the collaboration of intelligent agents without user assistance.",
            category="agent",
            key="derisk_agent_plan_planner_agent_profile_goal",
        ),
        expand_prompt=DynConfig(
            "Available Intelligent Agents:\n {{ agents }}",
            category="agent",
            key="derisk_agent_plan_planner_agent_profile_expand_prompt",
        ),
        constraints=DynConfig(
            [
                "Every step of the task reasoning_engine should exist to advance towards solving "
                ...
                "reasoning_engine. For the steps that are depended upon, consider the data they "
                ...
                "Utilize the provided resources to assist in generating the reasoning_engine "
                ...
                "Carefully review the reasoning_engine to ensure it comprehensively covers all "
...
assistants:[
    ...
    {{
        ...
        "rely": "1,2"
            ...
            category="agent",
            key="derisk_agent_plan_planner_agent_profile_examples",
        ),
    )
    _goal_zh: str = (
