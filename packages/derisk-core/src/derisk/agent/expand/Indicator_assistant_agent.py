...
class IndicatorAssistantAgent(ConversableAgent):
    ...
    profile: ProfileConfig = ProfileConfig(
        name=DynConfig(
            "Indicator",
            category="agent",
            key="derisk_agent_expand_indicator_assistant_agent_profile_name",
        ),
        role=DynConfig(
            "Indicator",
            category="agent",
            key="derisk_agent_expand_indicator_assistant_agent_profile_role",
        ),
        goal=DynConfig(
            ...
            category="agent",
            key="derisk_agent_expand_indicator_assistant_agent_profile_goal",
        ),
        constraints=DynConfig(
            ...
            category="agent",
            key="derisk_agent_expand_indicator_assistant_agent_profile_constraints",
        ),
        desc=DynConfig(
            "You can summarize provided text content according to user's questions and output the summaraization.",  # noqa
            category="agent",
            key="derisk_agent_expand_indicator_assistant_agent_profile_desc",
        ),
    )
